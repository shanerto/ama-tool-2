...
function sortActive(a: Event, b: Event) {
...
function sortPast(a: Event, b: Event) {
...
export default function AdminHomePage() {
  ...
  const companyEvents = events.filter((e) => e.status === "OPEN" && e.type === "company").sort(sortActive);
  const teamEvents = events.filter((e) => e.status === "OPEN" && e.type === "team").sort(sortActive);
  const pastEvents = events.filter((e) => e.status === "CLOSED").sort(sortPast);
      ...
      {loading ? (
        ...
        <div className="space-y-10">
          ...
          <section>
            ...
            {companyEvents.length === 0 ? (
              ...
              <ul className="space-y-3">
                {companyEvents.map((event) => renderEventRow(event))}
              </ul>
            )}
          </section>
          ...
          <section>
            ...
            {teamEvents.length === 0 ? (
              <p className="text-sm text-gray-400">No team events yet.</p>
            ) : (
              <ul className="space-y-3">
                {teamEvents.map((event) => renderEventRow(event))}
              </ul>
            )}
          </section>
